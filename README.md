![](https://capsule-render.vercel.app/api?type=waving&color=0:6d28d9&color=100:2563eb&height=180&section=header&text=SANJAI%20KUMAR%20T&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32&desc=Designer%20%7C%20Full-Stack%20Engineer%20%7C%20AI%20Enthusiast&descAlignY=62&descSize=16)

<a href="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3000&pause=1000&color=70A5FD&center=true&vCenter=true&width=750&lines=Networking
  +Cloud Computing;Passionate+about+AI+and+Data+Science;Frontend+Developer+%26+UI%2FUX+Designer;Always+learning%2C+always+growing">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3000&pause=1000&color=70A5FD&center=true&vCenter=true&width=750&lines=Building+beautiful%2B+functional+apps;Passionate+about+AI+and+Data+Science;Full-Stack+Developer+%26+UI%2FUX+Designer;Always+learning%2C+always+growing" />
</a>

---

## 👨‍💻 About Me

<img align="right" width="380" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif"/>

```python
class SanjaiKumar:
    def __init__(self):
        self.name = "Sanjai Kumar T"
        self.location = "Tamil Nadu, India"
        self.role = "Designer & Frontend Engineer"
        self.education = "B.Tech in AI & Data Science (2022-2026)"
        self.current_focus = "Generative AI & Web Development"
        
    def languages_spoken(self):
        return ["Python", "JavaScript", "HTML5", "CSS3", "English", "Tamil", "Hindi"]
    
    def tech_stack(self):
        return {
            "frontend": ["HTML5", "CSS3", "JavaScript", "Responsive Design"],
            "backend": ["Python", "Full-Stack Basics"],
            "ai_ml": ["Prompt Engineering", "Generative AI", "Computer Vision"],
            "design": ["Figma", "Adobe Photoshop", "Canva", "UI/UX Design"],
            "cloud": ["AWS", "Cloud Computing", "DevOps"],
            "networking": ["CCNA", "Linux (CentOS)", "Cisco Packet Tracer"]
        }
    
    def currently_learning(self):
        return ["Advanced Backend Development", "Cloud Architecture", "AI Model Optimization"]
    
    def fun_fact(self):
        return "I can turn ideas into pixels AND code! 🎨💻"
    
    def get_motto(self):
        return "Design with purpose. Code with passion. Ship with confidence."

sanjaidev = SanjaiKumar()
```

---

## 🛠️ Tech Stack

### 💻 Languages
![Python](https://img.shields.io/badge/Python-3776ab?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34C26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

### 🎨 Design & UI/UX
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)
![Adobe Photoshop](https://img.shields.io/badge/Adobe%20Photoshop-31A8FF?style=flat-square&logo=adobephotoshop&logoColor=white)
![Canva](https://img.shields.io/badge/Canva-00C4CC?style=flat-square&logo=canva&logoColor=white)

### 🤖 AI & Data Tools
![Generative AI](https://img.shields.io/badge/Generative%20AI-FF6B6B?style=flat-square&logo=openai&logoColor=white)
![Prompt Engineering](https://img.shields.io/badge/Prompt%20Engineering-4B8BBE?style=flat-square&logo=python&logoColor=white)

### ☁️ Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![DevOps](https://img.shields.io/badge/DevOps-0096D6?style=flat-square&logo=devops&logoColor=white)

### 🔧 Development Tools
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Cisco Packet Tracer](https://img.shields.io/badge/Cisco%20Packet%20Tracer-1BA0D7?style=flat-square&logo=cisco&logoColor=white)

---

## 💼 Professional Experience

### 🚀 Prompt Engineering Intern
**Pan Tech** | Jan 2025 - Present
- Developing expertise in prompt optimization for LLMs
- Creating intelligent AI-powered solutions
- Collaborating with cross-functional teams

### 🌐 Full-Stack Developer Intern
**Novi Tech** | Jan 2024 - Dec 2024
- Built responsive web applications using HTML, CSS, JavaScript
- Implemented frontend and basic backend logic
- Worked on real-world projects with production requirements

### 🎨 UI/UX Designer Intern
**Novi Tech** | Jan 2023 - Dec 2023
- Designed user-centric interfaces in Figma
- Created wireframes and high-fidelity mockups
- Collaborated with developers to ensure design implementation

---

## 🎯 Featured Projects

🌐 **Cisco Networking Fundamentals Project**
2026 | Networking | Infrastructure Design
Skills Used: Routing, Switching, VLANs, OSPF, DHCP, ACL, SSH, Cisco Packet Tracer
Description: Designed and configured network topologies using Cisco Packet Tracer
Key Highlights:
Implemented VLANs and Inter-VLAN communication
Configured DHCP and OSPF for automated addressing and routing
Applied ACLs and SSH to enhance network security
Performed connectivity testing and network troubleshooting

## 🎓 Education

### B.Tech - Artificial Intelligence and Data Science
**Bharath Niketan Engineering College** | Andipatti, Tamil Nadu | 2022-2026
- Specializing in AI, ML, and Data Science applications
- Coursework: Data Structures, Web Development, Computer Vision, Cloud Computing
- **Status:** Current Undergraduate (4th Year)

### Higher Secondary Education
**Padma Ramasamy Matric Hr. Sec. School** | Tamil Nadu | 2022

---

## 🏆 Certifications & Achievements

### 📜 Professional Certifications
- **Generative AI in Action** - IBM SkillBuild (2025)
- **Cloud Computing** - Besant Technologies (2026)
  - Coverage: CCNA, Linux (CentOS), DevOps
- **CCNA** - Cisco Certified Network Associate (In Progress)

### 🌟 Key Achievements
- Built and deployed multiple full-stack projects
- Hands-on experience with AI/ML and Generative AI tools
- Strong foundation in both code-based and no-code development
- Cross-functional expertise in design, development, and DevOps

---

## 🎨 Soft Skills

<div align="center">

| Communication | Problem Solving | Creativity |
|:---:|:---:|:---:|
| **Team Collaboration** | **Adaptability** | **Leadership** |

</div>

---

## 📊 GitHub Statistics

<div align="center">
  
  [![GitHub Stats](https://github-readme-stats.vercel.app/api?username=sanjaisk205&theme=tokyonight&show_icons=true&hide_border=true&count_private=true)](https://github.com/sanjaisk205)
  
  [![Most Used Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=sanjaisk205&layout=compact&theme=tokyonight&hide_border=true)](https://github.com/sanjaisk205)

</div>

---

## 🌍 What I'm Interested In

- 🎨 **Designing** - Creating beautiful, intuitive user experiences
- 🤖 **AI Tools** - Exploring the power of Generative AI and LLMs
- 📱 **Digital Content Creation** - Building engaging digital experiences
- 🌐 **Networking & Cloud** - Scaling applications globally
- 💡 **Innovative Solutions** - Solving real-world problems with technology

---

## 📚 Currently Learning

- Advanced backend architectures and microservices
- Optimizing AI models for production
- Cloud infrastructure and deployment strategies
- Advanced UI/UX design patterns

---

## 📫 Connect With Me

<div align="center">

[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sanjaisk205@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/sanjaisk205)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/sanjaisk205)
[![Phone](https://img.shields.io/badge/Phone-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](tel:+918838025035)

</div>

---

## 💬 My Motto

> **"Design with purpose. Code with passion. Ship with confidence."**

---

<div align="center">

### 🌟 If you find my work interesting, please consider starring my repositories!

![](https://capsule-render.vercel.app/api?type=waving&color=0:2563eb&color=100:6d28d9&height=100&section=footer&text=Thanks+for+visiting!&fontSize=20&fontColor=fff&animation=fadeIn)

</div>
